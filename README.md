...
- 对于图片高清和1px边框问题
	...
	实际要解决的是怎么让1个css像素是否正好覆盖1个物理像素，这样可以让同一张图片在不同dpr的手机上都能高清展示，1px边框在不同dpr的手机上一样细。
...
- 对于图片高清
  ...
  页面并没有缩放1/dpr，1个css像素还是覆盖多个物理像素。没有解决。
- 1px边框问题
...
我们写css样式就很方便，视觉稿中的尺寸除以100就好了，例如视觉稿中200px的元素，css样式是2rem。
